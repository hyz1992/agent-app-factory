...
/**
 * Claude UI/UX Pro Max 插件检查和安装脚本
 * 跨平台支持: Windows, macOS, Linux
 *
 * 使用方式：
 *   - node scripts/check-and-install-ui-skill.js
 *   - npm run install:ui-skill
 */
...
const PLUGIN_SCOPE = 'ui-ux-pro-max-skill';
const PLUGIN_ID = `${PLUGIN_NAME}@${PLUGIN_SCOPE}`;
const PLUGIN_MARKETPLACE = 'nextlevelbuilder/ui-ux-pro-max-skill';
...
function isPluginInstalled() {
  info('检查 ui-ux-pro-max-skill 插件...');
...
async function main() {
  console.log('');
  info('Claude UI/UX Pro Max 插件管理器');
  console.log('');
  ...
  try {
    ...
    if (isPluginInstalled()) {
      success('ui-ux-pro-max-skill 插件已安装');
      console.log('');
      return;
    }
    ...
    // 开始安装流程
    console.log('');
    warning('ui-ux-pro-max-skill 插件未安装，正在自动安装...');
    console.log('');
