...
7. **路径解析优先级**：当读取 Agent/Skill/Policy 文件时，按以下顺序查找：
   ...
   例如：
   - Agent 引用 `skills/prd/skill.md` 时，先尝试 `.factory/skills/prd/skill.md`，不存在则尝试 `skills/prd/skill.md`
...
某些 Agent 要求使用特定的技能来保证输出质量。你必须在验证这些 Stage 的产物时检查技能使用情况：
...
**验证方法**：
1. 检查 Agent 的输出消息中是否明确说明已使用 `superpowers:brainstorm` 技能
2. 如果 Agent 没有提及使用该技能，拒绝接受产物并要求重新执行
3. 提示："检测到未使用 superpowers:brainstorm 技能。请使用该技能深入挖掘用户想法后再生成 idea.md"

**失败处理**：
- 拒绝接受 `input/idea.md`
- 要求 Bootstrap Agent 重新执行，明确强调必须使用 `superpowers:brainstorm` 技能
- 如果连续两次失败，暂停流水线并请求人工介入

### ui 阶段

**强制要求**：必须使用 `ui-ux-pro-max` 技能生成设计系统

**验证方法**：
1. 检查 Agent 的输出消息中是否明确说明已使用 `ui-ux-pro-max` 技能
2. 检查 `ui.schema.yaml` 中的设计系统配置是否来自 `ui-ux-pro-max` 技能
3. 如果 Agent 没有提及使用该技能，或设计系统配置明显非专业推荐，拒绝接受产物

**失败处理**：
- 拒绝接受 UI 产物
- 要求 UI Agent 重新执行，明确强调必须使用 `ui-ux-pro-max` 技能
- 如果连续两次失败，暂停流水线并请求人工介入
...
2. 用户新开一个命令行窗口，执行：`factory continue`
3. 命令会自动启动新的 Claude Code 窗口并从下一个待执行阶段继续
...
每个 Stage 完成后，**必须**严格按以下格式向用户汇报，**不得简化或改变格式**：
...
**重要提醒**：
- 选项 2 必须始终显示，并标注"⭐ 推荐选项，节省 Token"
- 明确说明使用 `factory continue` 命令继续："在新的命令行窗口中执行：factory continue"
- 该命令会自动读取 `.factory/state.json` 并启动新的 Claude Code 窗口继续流水线
- 表格中的 `[下一阶段名称]` 和 `[当前阶段名称]` 需要替换为实际的阶段名称
- 表格中的 `[某个输入文件]` 需要替换为实际的输入文件路径

**输出检查清单**（在向用户展示前必须验证）：
- [ ] 使用了表格边框（┌─┬─┐ ┼ └─┴─┘）
- [ ] 第一行有标题框："📋 请选择下一步操作"
- [ ] 第二行有操作提示："输入选项数字（1-5），然后按回车键确认"
- [ ] 表格有"选项"和"说明"两列
- [ ] 所有 5 个选项都完整显示
- [ ] 选项 2 标注了"⭐ 推荐选项，节省 Token"
- [ ] 底部有 💡 提示框
- [ ] 没有简化为列表格式
